...
消息处理限制为普通任务同时执行 6 个、验证和部分管理任务同时执行 2 个；每个会话每类最多保留 32 个任务（含正在执行的任务），普通任务全局上限 256 个，验证和管理任务全局上限 128 个。空闲队列自动回收，超限的指令会尽量回复繁忙提示。抽奖管理与报名保持在同一队列，保证操作顺序。

延迟删消息使用 Redis 有序集合 `msgObjects:due`，每秒最多处理 100 条到期任务。升级时会自动分批迁移旧的 `msgObjects` 列表，无需手动清理 Redis。临时删除失败会保留任务重试；领取后未确认的任务会在 5 分钟后重新可用。

退出时先停止接收新任务、取消待执行任务和定时等待，再等待运行中的处理完成，最后关闭 Web、浏览器和数据库连接。退出最多等待 30 秒，超时会记录错误并以非零状态退出。批量签到期间再次启动批量签到会提示已有任务在执行。
