...
func ExportGachaHandle(update tgbotapi.Update) (bool, error) {
	...
	if len(players) > 1 {
		...
		// 绑定单个角色
		return Export(players[0].Uid, userAccount, chatId, utils.GetFullName(update.Message.From))
	}
	return true, nil
}

func Export(uid string, account account.UserAccount, chatId int64, name string) (bool, error) {
